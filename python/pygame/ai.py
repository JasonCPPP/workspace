    ...
    def process(self, time_passed):
        ...
        for key in keys:
            if key in  self.entities.keys():
                self.entities[key].process(time_passed_seconds)

    def render(self, surface):
        surface.blit(self.background, (0, 0))
        for key in self.entities.keys():
            self.entities[key].render(surface)

    def get_close_entity(self, name, location, range=100.):
        location = vector2d(*location)
        for key in self.entities.keys():
            if self.entities[key].name == name:
                distance = location.get_distance(self.entities[key].location)
                if distance < range:
